...
module Pod
    class TemplateConfigurator
        ...
        def clean_template_files
            ["./**/.gitkeep", "./**/.git", "./**/.gitignore", "configure", "_CONFIGURE.rb", "README.md","templates", "setup"].each do |asset|
                `rm -rf #{asset}`
            end
        end

        def replace_variables_in_files
            file_names = ['NAME.podspec'] + Dir.glob('**/**/*').select{ |f| File.file? f }
            file_names.each do |file_name|
                text = File.read(file_name)
                text.gsub!("${POD_NAME}", @pod_name)
                text.gsub!("${POD_DEMO_APP}", @pod_name + "Demo")
            ...
            replace_variables_in_swiftlint
        end

        def replace_variables_in_swiftlint
            if Dir.exists? "Example"
                file_name = 'Example/.swiftlint.yml'
                text = File.read(file_name)
                text.gsub!("${POD_NAME}", @pod_name)
                text.gsub!("${POD_DEMO_APP}", @pod_name + "Demo")
                File.open(file_name, "w") { |file| file.puts text }
